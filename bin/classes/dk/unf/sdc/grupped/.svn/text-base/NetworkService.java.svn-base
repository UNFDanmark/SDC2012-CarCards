...
public class NetworkService {
	...
	private NetworkService(BluetoothAdapter btAdapter) {
		mBluetoothAdapter = btAdapter;
		networkGame = false;
	}

	public static NetworkService getInstance(BluetoothAdapter btAdapter) {
		if (instance == null) {
			instance = new NetworkService(btAdapter);
		}
		return instance;
	}
			...
			while (true) {
				...
				try {
					...
					if (message.equals("GO!")) {
						...
						mHandler.obtainMessage(MainActivity.MESSAGE_WIN).sendToTarget();
					} else if(message.equals("lose")){
						mHandler.obtainMessage(MainActivity.MESSAGE_LOSE).sendToTarget();
					} else {
						String[] temp;
						/* delimiter */
						String delimiter = "-";
						...
						if (temp[0].equals("chosen")) {
							mHandler.obtainMessage(MainActivity.MESSAGE_CHOSEN,bytes, -1, buffer).sendToTarget();
						} else if (temp[0].equals("played")) {
							mHandler.obtainMessage(MainActivity.MESSAGE_PLAYED,bytes, -1, buffer).sendToTarget();
						}
					}
